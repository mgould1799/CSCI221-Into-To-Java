...
public class SearchNear {
    public static void main(String[] args) throws FileNotFoundException {
        ...
        PlaceInformation target = GeoCoder.findPlace(console.nextLine());
        if (target == null) {
            System.out.println("no matches for that search string.");
        } else {
            System.out.println("found " + target.getName());
            showMatches(data, console, target.getLocation());
        }
    }
    ...
    public static void showMatches(ArrayList<PlaceInformation> data,
        ...
        while (radius > 0) {
            ...
            if (radius > 0) {
                showEntries(data, console, target, radius);
            }
        }
    }
    ...
    public static void showEntries(ArrayList<PlaceInformation> data,
                                   Scanner console, GeoLocation target,
                                   double radius) {
        ...
        for (PlaceInformation entry : data) {
            ...
            if (nameMatch && tagMatch) {
                ...
                if (distance <= radius) {
                    System.out.printf("%.1f miles: %s %s\n", distance, 
